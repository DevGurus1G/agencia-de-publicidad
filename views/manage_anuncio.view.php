...
<form action="authenticate" method="post" id="registroAnuncio-form">
  <div class="configAnuncio">
    <? if (isset($anuncio)) :?>
      <a id="eliminarAnuncio" href="/anuncio/manage?borrar=<?= $anuncio[
        'id'
      ] ?>">Eliminar anuncio</a>

      <input type="hidden" name="editar_anuncio" id="editar_anuncio" value="<?= $anuncio[
        'id'
      ] ?>">
      <div class="imagen-input" id="imagen-input">
        ...
        <div class="imagen-grupo">
          <img src="data:image/jpeg;base64,<?= base64_encode(
            $imagen['imagen']
          ) ?>" alt="Foto del anuncio mostrado" class="avatar" id="avatar<?= $cont ?>" />
          <input type="file" name="imagen<?= $cont ?>" id="imagen<?= $cont ?>" />
          <label for="imagen<?= $cont ?>">Subir</label>
        </div>
        <? $cont++;?>
        <? endforeach;?>
      </div>

      <input type="text" placeholder="Titulo" value="<?= $anuncio[
        'titulo'
      ] ?>" id="tituloAnuncio"/>
